...
SELECT
  ...
  ROUND(
    100.0 * COUNT(DISTINCT e.anon_session_id)::numeric
         / NULLIF(COUNT(DISTINCT a.anon_session_id), 0)::numeric,
    1
  )::double precision AS engagement_rate_pct
FROM ask_flow_sessions a
LEFT JOIN engaged e
  ON e.day = a.day AND e.anon_session_id = a.anon_session_id
GROUP BY a.day
ORDER BY a.day DESC;